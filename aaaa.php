...
if(isset($_POST['ordersubmit'])) {
    if(strlen($_SESSION['login']) == 0) {   
        ...
        // Guardar los detalles del pedido en una sesión temporal
        $cartItems = [];
        $totalAmount = 0; // Aseguramos que el total sea 0 antes de empezar a sumar

        foreach($_SESSION['cart'] as $id => $item) {
            // Obtener el producto desde la base de datos para cada elemento del carrito
            $query = mysqli_query($con, "SELECT * FROM products WHERE id='$id'");
            $product = mysqli_fetch_array($query);

            // Calculamos el subtotal para el producto actual
            $subtotal = $item['quantity'] * $product['productPrice'] + $product['shippingCharge'];
            $totalAmount += $subtotal; // Sumamos el subtotal al total del carrito
        ...
        // Guardamos el total en la sesión para que esté disponible en payment-method.php
        $_SESSION['totalAmount'] = $totalAmount;

        // Guardamos los detalles del pedido en una sesión temporal
        $_SESSION['orderDetails'] = [
            'items' => $cartItems,
            'totalAmount' => $totalAmount
        ];

        // Redirige a la página de método de pago
        header('location:payment-method.php');
        exit();
    }
}

// Código para actualizar la dirección de facturación
...
// Código para actualizar la dirección de envío
...
?>
					...
					<td class="cart-product-sub-total"><span class="cart-sub-total-price"><?php echo "CLP"." ".$row['productPrice']; ?>.00</span></td>
<td class="cart-product-sub-total"><span class="cart-sub-total-price"><?php echo "CLP"." ".$row['shippingCharge']; ?>.00</span></td>
...
<div class="form-group">
 ...
 <div class="form-group">
					    <label class="info-title" for="Billing Pincode">Codigo Postal <span>*</span></label>
...
<div class="form-group">
						...
						<div class="form-group">
					    <label class="info-title" for="Billing State ">Estado del envío  <span>*</span></label>
...
<div class="col-md-4 col-sm-12 cart-shopping-total">
				...
				<tr>
					<td>
						<div class="cart-checkout-btn pull-right">
							<button type="submit" name="ordersubmit" class="btn btn-primary">ChekOut y Pagar</button>
	...
	<?php } else {
echo "Su carrito de compras está vacío";
		}?>
</div>			</div>
		</div> 
		</form>
